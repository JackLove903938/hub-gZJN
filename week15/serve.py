...
class EstateProjectRequest(BaseModel):
    """房地产楼盘三维分析请求。"""
    question: str
...
@app.post("/estate-analysis")
def estate_analysis(req: EstateProjectRequest):
    """SSE 流式：房地产楼盘三维分析（3 子代理并行）。"""
    import agents
    ...
    def event_stream():
        ...
        def run():
            try:
                import traceback
                r = agents.run_real_estate_project(
                    req.question,
                    on_main_step=on_main_step,
                    on_dispatch=on_dispatch,
                    on_subagent_step=on_subagent_step,
                ...
                logger.error(f"estate_analysis error:\n{err_detail}")
                push({"type": "error", 
                      "message": f"{type(e).__name__}: {str(e)[:300]}",
                      "traceback": err_detail[:1000]})
            finally:
                push(SENTINEL)
        ...
        yield "data: " + json.dumps({
            ...
            "mode": "estate_3d_analysis",
            "subagents": ["楼盘本体子代理", "配套规划子代理", "市场政策子代理"],
        }, ensure_ascii=False) + "\n\n"
        while True:
            ev = q.get()
            if ev is SENTINEL:
