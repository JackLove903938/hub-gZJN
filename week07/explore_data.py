...
def collect_stats(records: list) -> dict:
    ...
    for row in records:
        ...
        # 处理 CLUENER 格式：{"label": {"etype": {"surface": [[start, end]]}}}
        label = row.get("label")
        ...
        # 处理 peoples_daily 格式：{"ner_tags": ["O", "B-LOC", "I-LOC", ...]}
        ner_tags = row.get("ner_tags")
        if ner_tags and isinstance(ner_tags, list):
            i = 0
            while i < len(ner_tags):
                ...
                if tag.startswith("B-"):
                    ...
                    total_entities += 1
                    # 提取实体文本
                    entity_text = "".join(row.get("tokens", list(text))[start:end+1])
                    if etype not in entities_by_type:
                        entities_by_type[etype] = []
